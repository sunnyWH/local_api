...
class RB(Algo):
    ...
    def warmup(self):
        ...
        if self.vol < self.volThreshold and not self.volOK:
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.volumeOK and self.rangeOK:
                if (
                    self.lastVolCheck + timedelta(seconds=self.volFreq)
                    < self.currentTime
                ):
                    ...
                    if self.vol < self.volThreshold and not self.volOK:
                        ...
                        logging.info(f"NOT VolOK: {self.vol:.5f}, {self.volThreshold}")
                    self.lastVolCheck = self.currentTime.replace(
                ...
                if self.volOK:
                    ...
                    if self.position == 0:
                        ...
                        if (
                            ...
                            self.position += 1
                            self.lowReset = False
                if not self.volOK and self.position != 0:
                    self.flatten(self.latestTradePrice, "VOL_FLATTEN")
        ...
        self.buyStart = self.tickRound(
            (
                ...
                - max(
                    ...
                    (
                        self.range
                        * min(self.volThreshold, self.vol)
