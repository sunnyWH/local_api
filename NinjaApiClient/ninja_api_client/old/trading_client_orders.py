...
class TradingClient(NinjaApiClient):
                        ...
                        if self.currentTime > self.initialTradeTime + timedelta(
                            ...
                            if self.pnlCheckCounter != self.position:
                                ...
                                if np.mean(self.entryPrices) > self.latestTradePrice:
                                    ...
                                    self.position += 1
                                    self.order(self.ask, self.position - last_pos)
                                    self.logger.log_trade(
                                        self.currentTime.time(),
                                        self.ask,
                        ...
                        if self.currentTime > self.initialTradeTime + timedelta(
                            ...
                            if self.pnlCheckCounter != self.position:
                                ...
                                if np.mean(self.entryPrices) < self.latestTradePrice:
                                    ...
                                    last_pos = self.position
                                    self.position -= 1
                                    self.order(self.bid, self.position - last_pos)
                                    self.logger.log_trade(
                                        self.currentTime.time(),
                                        self.bid,
                                        self.position - last_pos,
    ...
    def flatten(self, price, tag):
        if self.position != 0:
            ...
            container.header.msgType = (
                NinjaApiMessages_pb2.Header.CANCEL_ALL_ORDERS_REQUEST
            )
            container.payload = ordercancel.SerializeToString()
            self.send_msg(container)
            # submit market order to flatten
            self.order(price, -self.position)
